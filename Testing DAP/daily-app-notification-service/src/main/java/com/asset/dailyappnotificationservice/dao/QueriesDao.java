...
public class QueriesDao
{
    ...
    public ConcurrentHashMap<Integer, String> getAll(){
        try {
            ...
            DailyAppLogger.INFO_LOGGER.debug(">>>>>> with query = {}", sqlQuery);
            return jdbcTemplate.query(sqlQuery.toString(), new AllQueriesExtractor());
        }catch (Exception ex){
            DailyAppLogger.ERROR_LOGGER.error(">>>>>> Queries RETRIEVING Error <<<<<<<");
            DailyAppLogger.DEBUG_LOGGER.error(">>>>>> Queries RETRIEVING Error <<<<<<<");
